...
如果你的文件名是 requirements.txt，就改成：
pip install -r requirements.txt

sudo apt update
sudo apt install -y python3-venv python3-pip

如果你是特定 Python 版本（如 3.11），可改为：
...
python -m pip install --upgrade pip
pip install -r requirement.txt
若文件名是 requirements.txt 则改成：
pip install -r ~/hwx/GCV5_SERVICE/requirement.txt -i https://pypi.tuna.tsinghua.edu.cn/simple