...
![alt text](日志pics/5.0软件架构.png)

### 任务
#### 1) 5.0添加界面旋转90度功能
#### 2) 电源未使能则以横线显示
#### 3) 在ESP32上实现GTB功能,通过HID/SPI/TCP与外部通信
#### 4) 在lubancat上实现采样任务并合到lua里
1)SPI通信添加

## 2026-03-13

### EPS32板子已拿到,DAP烧进去可以用DAP调试 移植GTB  TCP只需20几行代码
### PE0 PE1 这里加两个告警检测，一旦过流，这里就会被拉高，显示屏弹窗提示，不然到时候又有人问咋不输出了

### 2026-03-20
#### 1) 校准
#### 2) wifi 连GC上海,显示鲁班猫ip和5.0的IP
#### 3) 万用表界面,参考正点原子的
