...
# 2、如何取出buf
OTG_HS_IRQn这个中断最后会调用CUSTOM_HID_OutEvent这个函数，可以在这里取出buf
![alt text](USB_Related/image-14.png)

# 3 如何连续接收
在 DataOut 回调里重新挂起下一次接收可以进行连续接收
![alt text](USB_Related/image-15.png)

复合设备中
协议栈句柄（hUsbDevice）的classdata绑定了
实例句柄（hUsbDevice.pClassData_HID_Custom）

# 4 有时log不显示device->host的数据
bInterval 只在中断 IN 端点（设备→主机）生效：它告诉主机“最短轮询周期”，主机据此周期性向设备取数据
太快的话可能出错
